...
{
	//int tid = gettid();
	//printf("CREATING WORKER THREAD => TID: %d\n", tid);
...
{
	//int tid = gettid();
	//printf("CREATING MESSAGE THREAD => TID: %d\n", tid);
	struct thread_data *td = arg;
...
void send_hints_to_sched(struct queue* q, void* map_region, struct bento_ioc_enter_queue enter_queue, int fd) {
	
	struct sched_msg msg  = *((struct sched_msg *)((void *)map_region + q->offset));
	int main_tid = gettid();
        enter_queue.entries = (message_threads * worker_threads) + message_threads; 
	uint32_t cur_tid = main_tid+1;
	for (int i = 0; i < message_threads; i++) {
		// send hint for message thread
...
int main(int ac, char **av)
{
	...
	// aligning elems to next greatest power of 2, courtesy of StackOverflow :)
	...
	int n = (message_threads * worker_threads) + message_threads;
	n--;
	n |= n >> 1;
	n |= n >> 2;
	n |= n >> 4;
...
//	printf("ELEMS => %d\n", create_queue.elems);
	...
	fd = open("/sys/fs/ghost/enclave_10/ctl", O_RDWR);
       // printf("SHARED FILE FD => %d\n", fd);
	//printf("errno in opening up file %d\n", errno);
   ...
   // 	printf("q tail %d\n", q->tail);
	...
	// assign hints for every thread that will be created
	send_hints_to_sched(q, map_region, enter_queue, fd);

	enter_queue.entries = message_threads * worker_threads;
    	//ioctl(fd, GHOST_IOC_ENTER_QUEUE, (int32_t*) &enter_queue);
    	//ioctl(fd, EKIBEN_IOC_SEND_HINT, (int32_t*) &msg);
    	close(q_fd);
    	close(fd);
	...
	for (i = 0; i < message_threads; i++) {
		...
		pthread_cond_wait(&create_cond, &create_lock);
	}
