				     ...
				     unsigned long **output_counts)
{
	...
	for (i = 0; i < PLAT_NR && !is_last; i++) {
		...
		while (sum >= (plist[j] / 100.0 * nr)) {
			if (j == oval_len) {
				oval_len += 100;
				ovals = (unsigned int *)realloc(ovals, oval_len * sizeof(unsigned int));
				ocounts = (unsigned long *) realloc(ocounts, oval_len * sizeof(unsigned long));
			}
...
struct thread_data {
	...
	// arachne mutex -- TODO will this mess up the cacheline stuff? Is this required?
...
{
	struct request *ret = (struct request *) malloc(sizeof(*ret));
	...
	while (1) {
		...
		if (stopping) {
			for (i = 0; i < worker_threads; i++)
				...
				Arachne::signal(worker_threads_mem[i].tid);
			break;
		}
	}
...
again:
	...
	//	nop;
	//
	//printf("Trying to lock this cpu %d...\n", cpu);
	while (!per_cpu_locks[cpu].lock.try_lock())
		nop;
	...
	//printf("Locked this cpu %d\n", cpu);

	if (cur_cpu != cpu) {
		/* we got the lock but we migrated */
		//pthread_mutex_unlock(lock);
		per_cpu_locks[cpu].lock.unlock();
		goto again;
	}
	return cpu;
...
static void do_work(struct thread_data *td)
{
	...
	if (!skip_locking)
		cpu_lock = lock_this_cpu();
	for (i = 0; i < operations; i++)
		do_some_math(td);
	if (!skip_locking)
		per_cpu_locks[cpu_lock].lock.unlock();
		//pthread_mutex_unlock(lock);
}
...
void *message_thread(void *arg)
{
	...
	if (!worker_threads_mem) {
		...
		// TODO: may need to find some other way to exit
		Arachne::shutDown();
	...
	for (i = 0; i < worker_threads; i++) {
		pthread_t tid;
		worker_threads_mem[i].data = (unsigned long *) malloc(3 * sizeof(unsigned long) * matrix_size * matrix_size);
		worker_threads_mem[i].parent_tid = Arachne::getThreadId();
		...
		ret = Arachne::createThread(worker_thread, worker_threads_mem + i);
		if (ret == Arachne::NullThread) {
			fprintf(stderr, "error %d from pthread_create\n", ret);
			exit(1);
		}
		worker_threads_mem[i].tid = ret;
	}
	...
	for (i = 0; i < worker_threads; i++) {
		...
		Arachne::join(worker_threads_mem[i].tid);
	}
	return NULL;
}
					...
					unsigned long long *loop_runtime)
{
	...
	*loop_runtime = 0;
	...
	//printf("Yo what are the wakup stats?\n"); 
}
...
int test_start(int ac, char **av)
{
	...
	per_cpu_locks = (struct per_cpu_lock *) calloc(num_cpu_locks, sizeof(struct per_cpu_lock));
	if (!per_cpu_locks) {
	...
	for (i = 0; i < num_cpu_locks; i++) {
		...
		//per_cpu_locks[i].lock = Arachne::SpinLock::SpinLock(std::to_string(i).c_str());
		//per_cpu_locks[i].lock = Arachne::SpinLock();
	...
	message_threads_mem = (struct thread_data *) calloc(message_threads * worker_threads + message_threads,
	...
	for (i = 0; i < message_threads; i++) {
		...
		ret = Arachne::createThread(message_thread, message_threads_mem + index);
		if (ret == Arachne::NullThread) {
			fprintf(stderr, "error %d from pthread_create\n", ret);
			exit(1);
		}
		message_threads_mem[index].tid = ret;
	}
	...
	for (i = 0; i < message_threads; i++) {
		...
		Arachne::signal(message_threads_mem[index].tid);
		//pthread_join(message_threads_mem[index].tid, NULL);
		//
		Arachne::join(message_threads_mem[index].tid);
	}
	memset(&wakeup_stats, 0, sizeof(wakeup_stats));
	memset(&request_stats, 0, sizeof(request_stats));
...
void AppMainWrapper(int argc, char** argv) {
